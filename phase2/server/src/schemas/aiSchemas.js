import { z } from "zod";
import { isAllowedAssetSource } from "../utils/assetSource.js";
...
const assetSrcSchema = z
  ...
  .refine(
    ...
    "Asset source must reference an uploaded asset",
  );
...
const image = base.extend({
  ...
  // with designs created before assetId existed.
  ...
  .superRefine((doc, ctx) => {
    ...
    if (doc.elementOrder.length !== ids.size) {
      ctx.addIssue({
        code: "custom",
        path: ["elementOrder"],
        message: "elementOrder must contain every element exactly once",
      });

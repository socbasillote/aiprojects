...
export const exportRaster = (stage, document, format = 'png') => {
...
const elementToSvg = async (element) => {
  ...
  const stroke = element.stroke ? ` stroke="${escapeXml(element.stroke)}" stroke-width="${element.strokeWidth || 0}"` : ''
  const fill = escapeXml(element.fill || 'none')

  switch (element.type) {
    case 'text':
      return `<g transform="${transform}" opacity="${opacity}">${textToSvg(element)}</g>`
    case 'rect':
      return `<rect x="${element.x || 0}" y="${element.y || 0}" width="${element.width || 0}" height="${element.height || 0}" rx="${element.cornerRadius || 0}" fill="${fill}" opacity="${opacity}"${stroke}/>`
    case 'circle': {
      const radius = element.radius || Math.min(element.width || 100, element.height || 100) / 2
      return `<circle cx="${(element.x || 0) + radius}" cy="${(element.y || 0) + radius}" r="${radius}" fill="${fill}" opacity="${opacity}"${stroke}/>`
    }
    case 'ellipse': {
      const rx = (element.width || 120) / 2
      const ry = (element.height || 80) / 2
      return `<ellipse cx="${(element.x || 0) + rx}" cy="${(element.y || 0) + ry}" rx="${rx}" ry="${ry}" fill="${fill}" opacity="${opacity}"${stroke} transform="rotate(${element.rotation || 0} ${(element.x || 0) + rx} ${(element.y || 0) + ry})"/>`
    }
    case 'line': {
      const points = element.points || [0, 0, element.width || 100, 0]
      return `<line x1="${points[0] + (element.x || 0)}" y1="${points[1] + (element.y || 0)}" x2="${points[2] + (element.x || 0)}" y2="${points[3] + (element.y || 0)}" stroke="${escapeXml(element.stroke || '#111827')}" stroke-width="${element.strokeWidth || 3}" opacity="${opacity}"/>`
    }
    case 'image': {
      const href = await imageToDataUrl(element.src)
      return `<image href="${escapeXml(href || '')}" x="${element.x || 0}" y="${element.y || 0}" width="${element.width || 0}" height="${element.height || 0}" preserveAspectRatio="none" opacity="${opacity}" transform="rotate(${element.rotation || 0} ${(element.x || 0) + (element.width || 0) / 2} ${(element.y || 0) + (element.height || 0) / 2})"/>`
    }
    case 'svg':
      return `<g transform="${transform}" opacity="${opacity}"><image href="${escapeXml(await imageToDataUrl(element.src))}" width="${element.width || 0}" height="${element.height || 0}" preserveAspectRatio="none"/></g>`
    default:
      return ''
  }
}

...
export async function sendBookingConfirmation(input: { to: string; customer: string; business: string; service: string; date: string; time: string; paymentMethod: string; confirmationCode: string }) {
  ...
  try {
    qr = await QRCode.toDataURL(input.confirmationCode, { margin: 1, width: 220 });
  } catch (error) {
    console.error(`Unable to generate QR code for booking confirmation: ${input.confirmationCode}`, error);
    return { delivered: false, qr: "" };
  }

  if (!env.smtpHost || !env.smtpUser || !env.smtpPassword) {
    console.warn(`SMTP not configured; booking confirmation for ${input.to} was created. Code: ${input.confirmationCode}`);
    return { delivered: false, qr };
  }
  ...
  try {
    ...
    console.error(`SMTP delivery failed for booking confirmation ${input.confirmationCode} to ${input.to}`, error);
    return { delivered: false, qr };
  }
}

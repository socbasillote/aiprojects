  ...
  const filteredQuestions = useMemo(() => {
    ...
    return bankQuestions
      .filter((question) => {
        const matchesSearch =
          !query ||
          [question.title, question.text, question.topic]
            .join(" ")
            .toLowerCase()
            .includes(query);
        return (
          matchesSearch &&
          filterFields.every(
            ([field]) => !filters[field] || question[field] === filters[field],
          )
        );
      })
      .sort((first, second) =>
        sort === "title" ? first.title.localeCompare(second.title) : 0,
      );
    ...
    <AppShell>
        ...
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Reusable questions for your assessments
            </p>
          </div>
          <button
            type="button"
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <input
              ...
              className="min-w-64 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-slate-500"
            />
              ...
              >
                ...
                {uniqueValues(field).map((value) => (
                  <option key={value} value={value}>
                    {field === "type" ? questionTypeLabels[value] : value}
                  </option>
                ))}
              </select>
            ))}
              ...
              >
                ...
                <div className="mt-5 flex justify-end gap-2">
                  <button
                    ...
                    onClick={() =>
                      setNotice(
                        "Question editing is available from the paper after adding it.",
                      )
                    }
      ...
      {isCreateOpen && (
        <CreateQuestionDialog
          ...
          onCreated={(question) => {
            setBankQuestions((current) => [question, ...current]);
            setIsCreateOpen(false);
            setNotice("Question saved to your question bank.");
          }}
        />
      )}
    </AppShell>
